...
func TestDefaultTrayIconModeIsFlag(t *testing.T) {
	t.Parallel()

	if DefaultTrayIconMode != TrayIconModeFlag {
		t.Fatalf("default tray icon mode = %q, want %q", DefaultTrayIconMode, TrayIconModeFlag)
	}
}
